...
Базы очков задаются через `KVIZI_DIFFICULTY_POINTS`; неизвестные slug считаются
как `normal`, пока не добавлены в env.
...
- `/me` — личный счет
- `/top [topic_key]` — общая таблица лидеров или рейтинг по теме
...
Рекомендуемая частота: раз в 10-15 минут. Telegram также получает `open_period`
и автоматически закрывает poll по таймеру. `/cron/maintenance` синхронизирует
это состояние с SQLite. После остановки poll остаётся в статусе `closing` один
час, чтобы успели прийти `poll_answer`, задержанные webhook/proxy-сбоем. Если
Telegram сообщает больше голосов, чем сохранено в SQLite, ожидание продлевается
до суток; только после этого фиксируются unanswered challenge и анонсы.

Если `stopPoll` завершился неопределённым 503/proxy-сбоем, SQLite не считает poll
закрытым и не разрешает публиковать следующий вопрос в этом топике. Maintenance
повторит остановку при следующем запуске.
...
- новые difficulty-slug, например `ccna`, можно добавить через `KVIZI_DIFFICULTY_POINTS` и `KVIZI_CHALLENGE_REWARDS`
