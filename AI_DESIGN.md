...
Статус: **согласовано, ещё не реализовано**  
...
формирует только сервер. Безопаснее не просить ИИ переписывать всё сообщение:
модель генерирует лишь короткую подводку или реакцию, а неизменяемый блок фактов
добавляется кодом после неё.
...
Если ответ пустой, слишком длинный, содержит запрещённые данные или провайдер не
ответил вовремя, используется существующая фраза из `kvizi/copy.py`.
...
- интерфейс провайдера и `AIResult`;
- Groq, no-op/static fallback;
- конфигурация, лимиты, health/status;
- тесты timeout, ошибок и выключенных флагов;
- всё выключено по умолчанию.

### Этап 2. Голос ведущего

- короткие AI-подводки к безопасным структурированным фактам;
- неизменяемая серверная часть сообщения;
- fallback на `copy.py` при любой проблеме;
- поочерёдное включение событий.
